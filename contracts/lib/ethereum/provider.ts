...
const MAINNET_KEY = process.env.VITE_APP_MAINNET_KEY
...
const POLYGON_KEY = process.env.VITE_APP_POLYGON_KEY
...
const KOVAN_KEY = process.env.VITE_APP_KOVAN_KEY
...
export const fromEnvironment = () => {
  if (process.env.VITE_APP_STAGING_MATIC_USD) return kovan
  if (process.env.DEV) return local
  return polygon
}
